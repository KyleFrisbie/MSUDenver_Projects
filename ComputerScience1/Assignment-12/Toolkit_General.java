// File Toolkit_General, Version 03
...
public class Toolkit_General {

/** File Toolkit_General - a class with general tools, version nn
...
*/
   ...
   */
   ...
   public Toolkit_General(char outMode) {
   ...
   */
   public Toolkit_General(char outMode, FileWriter outputFileName) {
  ...
  */
   ...
   */
   ...
   /**
      selectionSortStringWithNumbers
      Sort a string or numbers array in ascending order. Keep its data, 
      the 2-dimensional array 'numbers' in the same row as the names get moved 
      around. This does NOT use indexes.
   ...
   */
    ...
    */
    ...
    int k, i;		// Indexes into the array to find the minimum value
    int minIndex; // Index of the minimum value in the array
    double minValue; // Lowest value in each pass in the sort
    ...
    isSortByString = (column < 0); // True means sort by String
    ...
    {    
       ...
       minIndex = k;  // Assume the minimum value is at location k
       minValue = numbers[k][0]; // Assume lowest value is in row l
       for (i = k + 1;  i < len;  i++) {
           if (isSortByString) {
               if (ary[i].compareTo(ary[minIndex]) < 0) minIndex = i;
           } else { // We are sorting by a specific column
               if (numbers[i][column] < minValue)       minIndex = i;
           } // End if (isSortString)
