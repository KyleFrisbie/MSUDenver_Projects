...
(defn get-impact
  ([impact preds tasks]
    (cond
      ...
      '()
      (empty? preds)
      (cons (gettime impact tasks) (get-impact (first (predecessors impact tasks)) (rest (predecessors impact tasks)) tasks))
      (> (gettime impact tasks) (gettime (first preds) tasks))
      (get-impact impact (rest preds) tasks)
      :else
      (get-impact (first preds) (rest preds) tasks))))

(defn start-day
  ([job tasks]
    (inc (reduce + (get-impact (first (predecessors job tasks)) (rest (predecessors job tasks)) tasks)))))

(println (start-day 'move_in tasks))
...
;
;(println (get-max '(frame roof select_subs move_in) tasks))
;
...
;    (inc (reduce + (get-impact (first (predecessors job tasks)) (rest (predecessors job tasks)) tasks)))))
;
;(println (critical-path 'get_permit tasks))
...
;      (empty? preds)
;      (cons critical (critical-path critical tasks))
;      (> (sum (predecessors critical tasks)) (sum (predecessors (first preds) tasks)))
;      (critical-path critical (rest preds) tasks)
;      :else
;      (critical-path (first preds) (rest preds) tasks))))
;
...
(defn start-day-retry-help
  ([crit-p tasks]
    (if (empty? crit-p)
      0
      (+ 1 (gettime (first crit-p) tasks) (start-day-retry-help (rest crit-p) tasks)))))

(defn start-day-retry
  ([job tasks]
    (start-day-retry-help (critical-path job tasks) tasks)))

(println (start-day-retry 'move_in tasks))
